...
2. Use the [Create a data refresh task](/reference/refresh_task_create) with the access\_token from the original connection.
3. Use webhooks, the [Retrieve a data refresh status](/reference/refresh_task) endpoint, or both to check the status of the refresh.
...
Truv has rate limits on each access\_token value. Within a 24-hour period, only an initial request and three refreshes are allowed. The `Refresh limit for the access_token exceeded` error occurs for any requests outside of the interval and limit.
...
* [Create a data refresh task](/reference/refresh_task_create)
* [Retrieve a data refresh status](/reference/refresh_task)
...
<CodeGroup>
  ```json Create a data refresh
  {
    "task_id": "48427a36d43c4d5aa6324bc06c692456"
  }
  ```

  ```json Retrieve a data refresh
  {

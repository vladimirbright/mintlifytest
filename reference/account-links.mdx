...
Links have an ID value and an access\_token. Together, these retrieve data from a user's account. Exchange the public\_token value during [authentication while implementing Truv Bridge](/docs/bridge-implementation#authentication). Access tokens are for sensitive operations, such as generating reports.
...
| status             | string | Status from [Connection Lifecycle](/reference/connection-lifecycle) related to account Link |
...
* [Retrieve a link token](/reference/link_exchange_token_flow)
* [List all links](/reference/links-list)
* [Retrieve a link](/reference/links-item)
* [Delete a link](/reference/delete_link_by_link_id)

...
An Order is a personal landing page created for a user to connect their account. These are personalized invites sent to a user's email or phone number. Create Orders with the [Truv Dashboard](https://dashboard.truv.com/app/orders) or programmatically using [API endpoints](#endpoints).
...
| source                 | string           | Type of platform, `internal`, `accio`, `tazworks`, `encompass`, `simplenexus`, `external_webpage`                                                                 |
| order\_number          | string           | External ID from client                                                                                                                                           |
| custom\_field          | string           | Client-provided custom field, can be used in email, SMS, or landing page*NOTE: This field is only available when enabled in the dashboard customization section.* |
| client\_name           | string           | Verification agency name                                                                                                                                          |
...
| insurance              | array of objects | Insurance verification metadata, see [Insurance object](#insurance-object)                                                                                        |
| bank                   | array of objects | Bank transactions metadata, see [Bank object](#bank-object)                                                                                                       |
| manager                | array of objects | Associated Order manager info, see [Manager object](#manager-object)                                                                                              |
| loan                   | array of objects | Loan information, see [Loan object](#loan-object)                                                                                                                 |
...
| access\_token    | string           | Access token to perform data refresh                                                                                                                                                                                                                |
...
| provider         | array of objects | Payroll or insurance provider, see [Provider object](#provider-object)                                                                                                                                                                              |
...
| company\_address | array of objects | Company address, see [Company address object](#company-address-object)                                                                                                                                                                              |
...
| link\_id         | string           | Link ID for connected account                                                                                                                                                                                                                       |
| access\_token    | string           | Access token to perform data refresh                                                                                                                                                                                                                |
| pdf\_report      | uri              | Verification report file as a PDF                                                                                                                                                                                                                   |
| data\_source     | string           | Source of data, see values below `payroll` - Payroll provider parsing`docs` - User uploaded documents`insurance` - Insurance data`financial_accounts` - Bank data`tax` - Tax documents`scoring_attributes` - Transactions scoring attributes report |
| provider         | array of objects | Payroll or insurance provider, see [Provider object](#provider-object)                                                                                                                                                                              |
| is\_suspicious   | Boolean          | Status of data from source marked as suspicious, such as if detecting fraud in uploaded documents or user SSN does not match data                                                                                                                   |
...
| Attribute | Type   | Description            |
| --------- | ------ | ---------------------- |
| email     | string | Email or Order manager |
...
* [Create an order](/reference/orders_create)
* [Retrieve an order](/reference/orders_read)
* [Create a data refresh order](/reference/orders_create_refresh_order)
* [Update an order](/reference/orders_partial_update)
* [Cancel an order](/reference/orders_cancel_create)

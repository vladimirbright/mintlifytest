...
<Info>
  Users and applicants require both `first_name` and `last_name` values when [testing](/docs/testing) for [Integrating Document Processing](/docs/implementing-income-and-employment-document-processing).
</Info>
...
* [List all users](/reference/users_list)
* [Create a user](/reference/users_create)
* [Retrieve a user](/reference/users_read)
* [Update a user](/reference/users_partial_update)
* [Delete a user](/reference/users_delete)
...
| client\_name         | string | Client name for Truv Bridge                                                  |
| product\_type        | string | Indicates main product type                                                  |
| allowed\_products    | array  | List of allowed products for account, only main type allowed if not included |
| company\_mapping\_id | string | Company ID to skip search step, see [Deeplinking](/docs/deeplinking)         |
| access\_token        | string | Access token of existing Link, used for data refresh                         |
